...
export const exportReceiptPDF = (receipt, org, theme, options = {}) => {
  ...
  const pageWidth = 210, pageHeight = 148, margin = 8, topMargin = 18
  let y = topMargin
  ...
  if (org?.address) { doc.text(safe(org.address), left1X, left1Y); left1Y += 3.5 }
  if (org?.phone) { doc.text(`Phone: ${safe(org.phone)}`, left1X, left1Y); left1Y += 3.5 }
  if (org?.gstin) { doc.text(`GSTIN: ${safe(org.gstin)}`, left1X, left1Y) }

  const right1X = box1X + box1Width - 4; let right1Y = box1Y + 3
  doc.setFontSize(8); doc.setFont('helvetica', 'normal')
  doc.text(`Receipt No: ${safe(receipt?.receipt_no)}`, right1X, right1Y, { align: 'right' }); right1Y += 4
  doc.text(`Date: ${safe(receipt?.receipt_date)}`, right1X, right1Y, { align: 'right' }); right1Y += 4
  doc.text(`Invoice Ref: ${safe(receipt?.invoice_number || 'N/A')}`, right1X, right1Y, { align: 'right' })
  y = box1Y + box1Height
  ...
  const payX = midX2 + 4; let payY = box2Y + 3
  doc.setFontSize(8); doc.setFont('helvetica', 'bold')
  doc.text('Payment Details', payX, payY); payY += 4
  doc.setFont('helvetica', 'normal')
  doc.text(`Amount: Rs. ${Number(receipt?.amount || 0).toFixed(2)}`, payX, payY); payY += 3.5
  doc.text(`Mode: ${safe(receipt?.payment_mode || 'N/A')}`, payX, payY); payY += 3.5
  doc.text(`Transaction No: ${safe(receipt?.transaction_no || 'N/A')}`, payX, payY)
  y = box2Y + box2Height

  // ---- ITEMS TABLE ----
  // Get items from the nested invoice structure
  const invoice = receipt?.invoices || {}
  let items = invoice?.invoice_items || []

  // If no items, create a fallback (should not happen now)
  ...
  autoTable(doc, {
    ...
    styles: { fontSize: 6, cellPadding: 1.2, textColor: 0, lineColor: 0, lineWidth: 0.1 },
