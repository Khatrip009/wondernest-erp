...
const ConductDemoModal = ({ open, demo, inquiryId, onClose }) => {
  ...
  const onOk = async () => {
    try {
      ...
      await conductMutation.mutateAsync({
        ...
        conductedAt: values.conducted_at ? values.conducted_at.format() : new Date().toISOString(),
      })
  ...
  return (
    <Modal
      title={<span style={{ color: primaryColor, fontFamily: fontHeading }}>Conduct Demo</span>}
      open={open}
      onOk={onOk}
      onCancel={onClose}
      confirmLoading={conductMutation.isLoading}
      destroyOnClose
      styles={{
    ...
    >
      ...
      >
        <Form.Item
          name="outcome"
          label={<span style={{ fontFamily: fontBody, color: primaryColor }}>Outcome</span>}
        ...
        >
          ...
          >
            ...
            <Select.Option value="Reschedule Needed">Reschedule Needed</Select.Option>
          </Select>
        </Form.Item>

        <Form.Item
          name="feedback"
          label={<span style={{ fontFamily: fontBody, color: primaryColor }}>Feedback</span>}
        >
        ...
        <Form.Item
          ...
          label={<span style={{ fontFamily: fontBody, color: primaryColor }}>Teacher Remarks</span>}
        >
          <TextArea rows={2} style={{ fontFamily: fontBody }} />
        </Form.Item>
        ...
        <Form.Item
          name="conducted_at"
          label={<span style={{ fontFamily: fontBody, color: primaryColor }}>Conducted At</span>}
        >
          <DatePicker showTime style={{ width: '100%', fontFamily: fontBody }} />
        </Form.Item>
      </Form>
    </Modal>
  )
